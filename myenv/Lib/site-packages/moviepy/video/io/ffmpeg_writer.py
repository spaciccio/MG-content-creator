...
def ffmpeg_write_video_byme(clip, filename, fps, root, percentuale_label, codec="libx264", bitrate=None,
                       preset="medium", withmask=False, write_logfile=False,
                       audiofile=None, verbose=True, threads=None, ffmpeg_params=None,
                       logger='bar'):
    """ Write the clip to a videofile. See VideoClip.write_videofile for details
    ...
    with FFMPEG_VideoWriter(filename, clip.size, fps, codec = codec,
        ...
        for t,frame in clip.iter_frames(logger=logger, with_times=True,
            ...
            # Aggiorna il testo del widget Label con la percentuale di caricamento
            percentuale_label.config(text=f"Percentuale di caricamento: {percentuale}%")
            percentuale_label.grid(row=19, column=0, sticky='ew', pady=10)
